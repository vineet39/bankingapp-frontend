...
//     if (!field || field <= 0 || field > loggedInUser.balance) {
...
function Withdraw() {
  ...
  const validationSchema = Yup.object().shape({
    amount: Yup.number()
      ...
      .integer("Amount must be an integer").max(balance, "Amount cannot be greater than current balance"),
  });
  ...
  const onSubmit = async (values, { setSubmitting, resetForm, setStatus }) => {
    ...
    try {
      ...
      setStatus("Failed to withdraw: " + err.message);
    }
